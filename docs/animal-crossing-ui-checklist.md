...
| # | 条目 | 为什么值 | 成本 | 优先级 |
|---|---|---|---|---|
| 1 | 按钮"物理下沉"（按下位移 + 阴影收缩） | 全局手感，改一处受益全站 | 极低 | **P0** |
| 2 | 确认/成功给"放射式"反馈 | 下载、追番、刷新这些关键操作现在只有 toast | 低 | **P0** |
| 3 | 数字滚动（CountUp）代替瞬变 | 速度/进度/体积数字很多，滚动立刻有"在跑"的感觉 | 低 | **P0** |
| 4 | 进度拟物化（机场信息屏式：翻牌 + 逐灯点亮） | 下载队列天生适合，比进度条有意思得多 | 中 | **P1** |
| 5 | loading 有内容（小飞机穿云 / 浮动小岛） | 我们的 spinner 是纯转圈；BT 聚合搜索等待时间长 | 中 | **P1** |
| 6 | 弹窗气泡式入场 + 静止时微形变 | `AcModal`/`AcDrawer` 一次改造全站生效 | 中 | **P1** |
| 7 | 状态横幅 + 图章（任务/订阅/规则命中） | 日历、RSS、下载页都用得上 | 中 | **P1** |
| 8 | 音效层（默认静音，设置里开） | 质感提升最明显，但必须默认关 | 中 | **P2** |
...
| 静止循环 | 6s 一轮的极轻微 `border-radius` / `scale` 呼吸（±0.5%） |
...
**AniDog 怎么用**：把 `AcSpinner` 升级成"场景化 loading"：
- BT 聚合搜索（多站点并发等待，耗时最长）→ 小飞机穿云式的多源探测动画（4 个源依次点亮）
- 扫描 / 整理文件 → 小岛式循环（缓慢、不催促）
- 普通接口等待 → 保留现在的 spinner，不要所有地方都上大动画
